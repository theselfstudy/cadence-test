...
// ============================================
// BUILDING INSIGHT PLACEHOLDER
// Shows encouraging message with growing tree icon
// and progress dots when section lacks enough data
// ============================================

interface BuildingInsightPlaceholderProps {
  /** Current number of unique days logged */
  uniqueDaysLogged: number;
  /** Days needed to unlock this insight (default 14) */
  daysNeeded?: number;
  /** Encouraging title for this specific section */
  title: string;
  /** Encouraging subtitle explaining what users will see */
  subtitle: string;
}

export function BuildingInsightPlaceholder({
  uniqueDaysLogged,
  daysNeeded = 14,
  title,
  subtitle,
}: BuildingInsightPlaceholderProps) {
  const daysRemaining = Math.max(0, daysNeeded - uniqueDaysLogged);

  return (
    <div className="bg-app-cream/30 rounded-lg p-4">
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div>
          <h3 className="font-medium text-app-charcoal">{title}</h3>
          <p className="text-sm text-app-gray mt-0.5">{subtitle}</p>
        </div>
      </div>

      {/* Progress section */}
      <div className="bg-app-white/70 rounded-lg p-3">
        ...
        <div className="flex items-center gap-1 mt-3 flex-wrap">
          {Array.from({ length: daysNeeded }).map((_, i) => (
            <div
              key={i}
              className={`w-2.5 h-2.5 rounded-full ${
                i < uniqueDaysLogged ? "bg-app-teal" : "bg-app-border"
              }`}
            />
          ))}
        </div>
        <p className="text-xs text-app-gray mt-2">
          {uniqueDaysLogged} of {daysNeeded} days
        </p>
      </div>
    </div>
  );
...
// ============================================
// GROWING TREE ICON
// Copied from TrustBanner for consistency
// ============================================
